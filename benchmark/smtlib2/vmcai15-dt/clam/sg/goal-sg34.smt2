...
(assert (forall ((n Nat) (m Nat)) (= (qmult zero n m) m) ))
(assert (forall ((n Nat) (m Nat) (p Nat)) (= (qmult (succ n) m p) (qmult n m (plus p m))) ))
(assert (forall ((x Nat) (y Nat) (z Nat)) (= (plus (mult x y) z) (qmult x y z)) ))
(assert (not (forall ((x Nat) (y Nat)) (= (mult x y) (qmult x y zero)) )))
(check-sat)
(exit)
