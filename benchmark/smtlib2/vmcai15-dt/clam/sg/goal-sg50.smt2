...
(assert (forall ((x Nat) (y Lst)) (= (count x (insort x y)) (succ (count x y))) ))
(assert (forall ((x Nat) (y Nat) (z Lst)) (=> (not (= x y)) (= (count x (insort y z)) (count x z))) ))
(assert (not (forall ((x Nat) (y Lst)) (= (count x (sort y)) (count x y)) )))
(check-sat)
(exit)
