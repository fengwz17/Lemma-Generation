...
(assert (forall ((x Lst) (y Lst)) (= (len (append x y)) (plus (len x) (len y))) ))
(assert (forall ((x Lst) (y Lst)) (= (len (qreva x y)) (plus (len x) (len y))) ))
(assert (forall ((x Lst)) (= (len (qrev x)) (len x)) ))
(assert (forall ((x Lst) (y Lst)) (= (plus (len x) (len y)) (qlen (queue x y))) ))
(assert (forall ((x Lst) (y Nat)) (= (succ (len (butlast (cons y x)))) (len (cons y x))) ))
(assert (forall ((q Queue) (n Nat)) (= (qlen (enqueue q n)) (succ (qlen q))) ))
(assert (forall ((q Queue) (n Nat)) (=> (and (isAmortized q) (not (isEmpty q))) (= (succ (qlen (qpop q))) (qlen q))) ))
(assert (forall ((x Lst)) (= (append x nil) x) ))
(assert (forall ((x Lst) (n Nat) (y Lst)) (= (butlast (append x (cons n y))) (append x (butlast (cons n y)))) ))
(assert (forall ((x Lst) (y Lst) (z Lst)) (= (qreva (qreva x y) z) (qreva y (append x z))) ))
(assert (forall ((x Lst) (y Lst) (z Lst)) (= (append x (append y z)) (append (append x y) z)) ))
(assert (forall ((x Lst) (y Lst)) (= (queue-to-lst (queue x y)) (queue-to-lst (amortizeQueue x y))) ))
(assert (forall ((x Lst) (y Lst)) (= (butlast (queue-to-lst (queue x y))) (queue-to-lst (qpop (queue x y)))) ))
(assert (not (forall ((y Lst) (x Lst) (n Nat)) (= (append (queue-to-lst (queue x y)) (cons n nil)) (queue-to-lst (enqueue (queue x y) n))) )))
(check-sat)
(exit)
