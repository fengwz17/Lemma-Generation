...
(assert (forall ((x Nat)) (= (ztake x znil) znil) ))
(assert (forall ((x ZLst)) (= (ztake zero x) znil) ))
(assert (forall ((x Nat) (y Pair) (z ZLst)) (= (ztake (succ x) (zcons y z)) (zcons y (ztake x z))) ))
(assert (not (forall ((n Nat) (xs Lst) (ys Lst)) (= (ztake n (zip xs ys)) (zip (take n xs) (take n ys))) )))
(check-sat)
(exit)
