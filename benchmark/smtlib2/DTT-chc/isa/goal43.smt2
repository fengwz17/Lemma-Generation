...
(assert (forall ((x Int) (y Lst)) (= (dropWhile (cons x y)) (ite (P x) (dropWhile y) (cons x y)))))
(assert (= (takeWhile nil) nil))
(assert (forall ((x Int) (y Lst)) (= (takeWhile (cons x y)) (ite (P x) (cons x (takeWhile y)) nil))))
(assert (not 
(forall ((xs Lst)) (= (append (takeWhile xs) (dropWhile xs)) xs)) ; G43 
))
(check-sat)
